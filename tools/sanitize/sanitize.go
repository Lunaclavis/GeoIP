...
func main() {
	...
	if outputDir != "" {
		isExist, err := checkDir(outputDir)
		if err != nil {
			log.Fatalf("Error: %v\n\n", err)
		}
		if !isExist {
...
func checkDir(dir string) (bool, error) {
	...
	return true, fmt.Errorf("Path %q is not a valid directory", dir)
}
...
func cleanFile(path string) (*strings.Builder, error) {
	...
	if fileSize > maxFileSize {
		return nil, fmt.Errorf("File %q is too large", path)
	}
